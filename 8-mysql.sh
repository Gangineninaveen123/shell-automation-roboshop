...
echo "Password:: $MYSQL_ROOTPASSWORD"
...
#Installing mysqlo server
dnf install mysql-server -y &>> $LOG_FILE
VALIDATE $? "Installing mysql server"

#Enabling abd starting mysql bd server
systemctl enable mysqld &>> $LOG_FILE
systemctl start mysqld &>> $LOG_FILE
VALIDATE $? "Enabling and starting mysql db"
...
mysql_secure_installation --set-root-pass $MYSQL_ROOTPASSWORD &>> $LOG_FILE
